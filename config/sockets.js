...
module.exports.sockets = {
  ...
  ***************************************************************************/
  afterDisconnect: function(session, socket, cb) {
    // Remove user from the connections list
    if(typeof session.passport !== 'undefined') {
      Connections.removeUser(session.passport.user);

      setTimeout(function () {
        if(typeof session.passport.user != 'undefined' && ! Connections.isOnline(session.passport.user)) {
          sails.sockets.broadcast('users_online', 'user_disconnected', session.passport.user);
        }
      }, 3000);
    }
